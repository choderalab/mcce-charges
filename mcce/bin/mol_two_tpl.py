	...
	for conf in protonation_states.ligand_list:
		...
		for atom_name in atoms_list[count_three:count_four]: 
			if len(atom_name.resName) < 1:
				...
				tpl.write(template.format("CHARGE",conf, \
                                  atom_name.name, atom_name.charge))
				for item in bond_list[count_three:count_four]:
					fields = item.split()
					if item != '@<TRIPOS>BOND\n':
						#print type(fields[2])
						if int(fields[1]) == atom_name.serial or int(fields[2]) == atom_name.serial:
							print str(atom_name.serial) + '  ' +str(atom_name.name)
				
				#print str(conf) + str(atom_name.name)  
